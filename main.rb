...
DB = Sequel.connect(adapter: :postgres, user: ENV["DB_USERNAME"], password: ENV["DB_PASSWORD"], host: 'localhost', port: 5432, database: ENV["DB_NAME"], max_connections: 10, logger: Logger.new("#{__dir__}/log/db.log"))
...
loop do
  begin
    parsed_data = Nokogiri::HTML5(request(url: ENV["OLX_URL"]))
    ...
    if !!parsed_data
      ...
      app_logger.info "No updates"
    end
    sleep 30
  rescue => e
    app_logger.fatal "Error #{e}"
    Telegram.bot.send_message chat_id: ENV["TELEGRAM_MAIN_CHAT_ID"], text: "There is an error #{e}, try to restart bot by /start"
  end
end
